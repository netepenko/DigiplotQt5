# reading and analyzing digitizer plots
#
...
#----------------------------------------------------------------------
# dialog to enter numerical data, using a validator
#----------------------------------------------------------------------

# veryfies that a number has been entered
...
class PlotFrame(QtWidgets.QMainWindow):
     ...
     def RatesForAllChan(self,ftw):
         self.OnClear()
         try:
             self.LoadParameters("C://Users//Alex//Desktop//par.data")
         except:
             pass
         for i in range(0,4):
     ...
     def OnFitHistogram(self):
         ...
         self.show_histos(self.current_histo)
         h.plot_fit(axes =  self.histoframe.axes)
         self.histoframe.axes.set_xlim((cmin,cmax)) # reset the last histogram limits
 ...
 #             V = self.V_slice[i]
     ...
     def OnTSFFTplotps(self,event):
          ...
          if self.par["draw_lines"]:
               ...
               self.my_plot(self.fft.f,self.fft.logp)
          else:
               #self.axes.plot(self.fft.f,self.fft.logp, '.')
               self.my_plot(self.fft.f,self.fft.logp, '.')
     ...
     def OnFFTplotps(self):
          ...
          if self.par["draw_lines"]:
               #self.axes.plot(self.fft.f,self.fft.logp)
               self.all_plot = self.my_plot(self.fft.f,self.fft.logp)
          else:
               #self.axes.plot(self.fft.f,self.fft.logp, '.')
               self.all_plot = self.my_plot(self.fft.f,self.fft.logp, '.')
          # self.axes.set_yscale('log')
     ...
     def OnFFTplotan(self):
          ...
          if self.par["draw_lines"]:
               ...
               self.all_plot = self.my_plot(self.fft.f,np.abs(self.fft.an))
          else:
               #self.axes.plot(self.fft.f,np.abs(self.fft.an),'.')
               self.all_plot = self.my_plot(self.fft.f,np.abs(self.fft.an),'.')
          self.axes.set_yscale('log')
     ...
     def OnFFTplotbn(self):
          ...
          if not self.fft.ok:
               ...
               return
          if self.par["draw_lines"]:
               # mself.axes.plot(self.fft.f,np.abs(self.fft.bn))
               self.all_plot = self.my_plot(self.fft.f,np.abs(self.fft.bn))
          else:
